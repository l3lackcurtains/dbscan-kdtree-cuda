...
__device__ __host__ inline double kdTree::dist(kdNode *a, kdNode *b)
{
  double t, d = 0;
  int dim = DIMENSION;
  while (dim--)
...
__device__ __host__ inline void kdTree::swap(kdNode *x, kdNode *y)
{
...
__device__ __host__ kdNode *kdTree::findMedian(kdNode *start, kdNode *end,
                           int idx)
{
  if (end <= start)
    return NULL;
...
__device__ __host__ kdNode *kdTree::buildTree(kdNode *t, int len, int i)
{
  kdNode * n;
  if (!len)
    return 0;
...
__device__ __host__ kdNode kdTree::getKdRoot() {
  return *this->kdRoot;
}
...
__global__ void kernel(kdTree *kdGpu)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;

  if (i == 0)
  {
    kdNode kk = kdGpu->getKdRoot();
    printf("Hellooo...%d\n", kk.id);
  }
}
...
int main(int argc, char **argv)
{
  ...
  kdGpu->buildTree(wp, DATASET_COUNT, 0);

   cudaDeviceSynchronize();

  kernel<<<dim3(THREAD_BLOCKS, 1), dim3(THREAD_COUNT, 1)>>>(kdGpu);
